...
(define n-c-alto-ventana-total 100)
...
(define fondo-0 (freeze (empty-scene ancho-fondo-ventana-real alto-fondo-ventana transparente)))
...
(define (lista-suelo n e)
  (cond
    ...
    ((< (+ n 10) (+ n-c-ancho-ventana n-c-ancho-ventana-aumentado))
     (lista-suelo (+ n 1) (cons (punto-c n (- n-c-alto-ventana 5)) e)))
    (else
     e)))
...
(define (redefinir-tamaños x)
  ...
  (set! ancho-fondo-ventana-real (* l-cadrado (+ n-c-ancho-ventana n-c-ancho-ventana-aumentado)))
  (set! alto-fondo-ventana (* l-cadrado n-c-alto-ventana))
  (set! fondo-0 (freeze (empty-scene ancho-fondo-ventana-real alto-fondo-ventana transparente)))
  ...
  (set! empty-s 
        (cond
          ((file-exists? "fondo-c3.jpg")
           (freeze (escalar (* ancho-fondo-ventana 1.2) alto-fondo-ventana (bitmap "fondo-c3.jpg"))))
          (else
           (freeze (rectangle ancho-fondo-ventana alto-fondo-ventana "solid" "white")))))
  ...
  (set! linea-alto (freeze (line 0 alto-fondo-ventana "lightgray")))
  (set! linea-ancho (freeze (line ancho-fondo-ventana-real 0 "lightgray")))
  (set! cadro-r (freeze
                 (overlay
...
(define (calc-apart-vect pc-y pc-x)
  (cond
    ((or
      (< pc-x 0)
      (>= pc-x n-c-ancho-ventana-real))
     -1)
    (else
     (+ (* pc-y n-c-ancho-ventana-real) pc-x))))
  ...
  (append
   ...
   (if (not (<= pc-x 0))
       (append
        ...
        (s-vector-ref vector (calc-apart-vect (+ pc-y 2)  (- pc-x 1))))
       empty)
   (if (not (>= pc-x n-c-ancho-ventana-real))
       (append
   ...
   (if (equal? (pj-estado-s (xogo-pj x)) "down")
       (append 
        ...
        (if (< pc-x n-c-ancho-ventana-real) 
            (s-vector-ref vector (calc-apart-vect (+ pc-y 3) (+ pc-x 1))) empty))
       empty)
...
(define (coller-puntos-adecuados-x P-O punto-c-x-pj)
  (cond
    ...
    ((= punto-c-x-pj (- (+ n-c-ancho-ventana n-c-ancho-ventana-aumentado) 1))
     (append (vector-ref P-O punto-c-x-pj) (vector-ref P-O (inexact->exact (- punto-c-x-pj 1)))))
    ((= punto-c-x-pj (+ n-c-ancho-ventana n-c-ancho-ventana-aumentado))
     (vector-ref P-O (inexact->exact (- punto-c-x-pj 1))))
    ((> punto-c-x-pj (+ n-c-ancho-ventana n-c-ancho-ventana-aumentado))
     empty)
    (else
...
(define linea-alto (freeze (line 0 alto-fondo-ventana "lightgray")))
(define linea-ancho (freeze (line ancho-fondo-ventana-real 0 "lightgray")))

(define (cuadricula-filas py fondo)
  (place-image
   linea-ancho
   (/ ancho-fondo-ventana-real 2) py
   (cond
     ((>= py alto-fondo-ventana)
      fondo)
     (else
...
(define (cuadricula-columnas px fondo)
  (place-image
   linea-alto
   px (/ alto-fondo-ventana 2)
   (cond
     ((>= px ancho-fondo-ventana-real)
      fondo)
     (else
        ...
        (place-image tipo-cadro
                     (cond
                       ((< (punto-x (pj-punto (xogo-pj x))) (/ ancho-fondo-ventana 2))
                        (punto-x (punto-cadro->punto (car lista))))
                       ((> (punto-x (pj-punto (xogo-pj x))) (- ancho-fondo-ventana-real (/ ancho-fondo-ventana 2)))
                        (- (punto-x (punto-cadro->punto (car lista))) (- ancho-fondo-ventana-real ancho-fondo-ventana)))
                       (else
...
(define (barra-carga x)
  (rectangle (* ancho-fondo-ventana (* (xogo-carga x) 0.01)) (* l-cadrado 4) "solid" "lightblue"))

; fondo da pantalla inicia

(define (fondo-menu)
  (rectangle ancho-fondo-ventana (* l-cadrado 4) "solid" (make-color 230 230 230)))

; pantalla inicial

(define (pantalla-inicio x)
  (place-image (text "MUNDO DE CADRADOS" (floor (* (* l-cadrado 1.5) (/ n-c-ancho-ventana 35))) "black")
               (/ ancho-fondo-ventana 2) (* l-cadrado 2)
               (place-image 
                (line ancho-fondo-ventana 0 "black")
                (/ ancho-fondo-ventana 2) (* l-cadrado 4)
                (place-image 
                 (line ancho-fondo-ventana 0 "black")
                 (/ ancho-fondo-ventana 2) (- alto-fondo-ventana (* l-cadrado 4))
                 (cond
                   ((equal? (xogo-estado x) "cargando")
                    (overlay 
                     ...
                     (place-image
                      (barra-carga x)
                      (/ ancho-fondo-ventana 2) (/ (* l-cadrado 4) 2)
                      (empty-scene ancho-fondo-ventana alto-fondo-ventana))))
                   (else
                    (place-image (boton-xogar x)
                                 (/ ancho-fondo-ventana 2) 
                                 (- (/ alto-fondo-ventana 2.5) (* (/ n-c-ancho-ventana 35) (* l-cadrado 2)))
                                 (overlay (rectangle (- ancho-fondo-ventana 1) (- alto-fondo-ventana 1) "outline" "black")
                                          (place-image
                                           ...
                                           (place-image 
                                            (fondo-menu)
                                            (/ ancho-fondo-ventana 2) 
                                            (- alto-fondo-ventana (* l-cadrado 2))
                                            (empty-scene ancho-fondo-ventana alto-fondo-ventana)))))))))))
                            ...
                            (place-image
                             ...
                             (cond 
                               ((<= punto-pj-x (/ ancho-fondo-ventana 2))
                                (+ (/ ancho-fondo-ventana 2) (/ (- ancho-fondo-ventana-real ancho-fondo-ventana) 2)))
                               ((>= punto-pj-x (- ancho-fondo-ventana-real (/ ancho-fondo-ventana 2)))
                                (- (/ ancho-fondo-ventana 2) (/ (- ancho-fondo-ventana-real ancho-fondo-ventana) 2)))
                               (else
                                (- (+ (/ ancho-fondo-ventana 2) (/ (- ancho-fondo-ventana-real ancho-fondo-ventana) 2)) 
                             ...
                             (place-image 
                              ...
                              (cond
                                ((> punto-pj-x (- ancho-fondo-ventana-real (/ ancho-fondo-ventana 2)))
                                 (- (/ ancho-fondo-ventana 2)
                                    (/ (- (- ancho-fondo-ventana-real (/ ancho-fondo-ventana 2)) (/ ancho-fondo-ventana 2)) 30)))
                                ((> punto-pj-x (/ ancho-fondo-ventana 2))
                                 (- (/ ancho-fondo-ventana 2)
...
(define empty-s 
  (cond
    ((file-exists? "fondo-c3.jpg")
     (freeze (escalar (* ancho-fondo-ventana 1.2) alto-fondo-ventana (bitmap "fondo-c3.jpg"))))
    (else
     (freeze (rectangle ancho-fondo-ventana alto-fondo-ventana "solid" "lightgray")))))
           ...
           (cond
             ...
             ((>= punto-pj-x (- ancho-fondo-ventana-real (/ ancho-fondo-ventana 2)))
              (- (if cambiando-tamaño
                     (* (/ punto-pj-x l-cadrado-a) l-cadrado)
                     punto-pj-x)
                 (- ancho-fondo-ventana-real ancho-fondo-ventana)))
             (else
              (/ ancho-fondo-ventana 2)))
           (cond
             (#t
...
(define (fora? x)
  (cond
    ...
    (else
     (define punto-pj-y (punto-y (pj-punto (xogo-pj x))))
     (> (- punto-pj-y (* l-cadrado 3)) alto-fondo-ventana))))
...
(define (clic-xogo? px py)
  (and
   ...
   (<= px (+ (/ ancho-fondo-ventana 2) (/ (* (* l-cadrado 5) (/ n-c-ancho-ventana 35)) 2)))
   (>= py (- (- (/ alto-fondo-ventana 2.5) (* (/ n-c-ancho-ventana 35) (* l-cadrado 2)))
     ...
     (define px-v 
       (cond 
         ((<= punto-pj-x (/ ancho-fondo-ventana 2))
          px)
         ((>= punto-pj-x (- ancho-fondo-ventana-real (/ ancho-fondo-ventana 2)))
          (+ px (- ancho-fondo-ventana-real ancho-fondo-ventana)))
         (else
...
(define (roda-adiante x)
  (cond
    ((and
      (< (xogo-conm x) 3)
      (> n-c-ancho-ventana 45)
      (not (member (xogo-estado x) (list "inicio" "cargando" "xogo->pantallas"))))
     (set! zoom (+ zoom 1))
     (set! l-cadrado-a l-cadrado)
     (set! l-cadrado (+ l-cadrado 1))
     (set! n-c-ancho-ventana-aumentado 
           (inexact->exact (round (- (/ ancho-fondo-ventana-real l-cadrado) (/ ancho-fondo-ventana l-cadrado)))))
     (set! n-c-ancho-ventana (round (/ ancho-fondo-ventana l-cadrado)))
     (set! n-c-ancho-ventana-real (round (+ n-c-ancho-ventana n-c-ancho-ventana-aumentado)))
