...
(define (tecla x t)
  ...
  (when (and (equal? (xogo-estado x) "edicion") (equal? t #\return))
    (write-to-file (puntos->texto lista-puntos-creados) "mapa-edicion.txt"))
  (cond
                   ...
                   [struct-copy pj (xogo-pj x)
                                [punto
                                 (punto
                                  (cond
                                    ((and
                                      (> (punto-x (xogo-p-mouse x)) (+ (/ ancho-fondo-ventana 2) (/ ancho-fondo-ventana 4)))
                                      (< punto-pj-x (- ancho-fondo (/ ancho-fondo-ventana 2))))
                                  ...
                                  (cond
                                    ((and
                                      (> (punto-y (xogo-p-mouse x)) (+ (/ alto-fondo-ventana 2) (/ alto-fondo-ventana 5)))
                                      (< punto-pj-y (- alto-fondo (/ alto-fondo-ventana 2))))
...
(define (roda-adiante x)
  (cond
    ((and
      ...
      (not (member (xogo-estado x) (list "inicio" "cargando" "xogo->pantallas" "xogo->edicion" "sel-pantallas-e" "edicion->atras" "edicion->nuevo")))
      )
     (set! zoom (+ zoom 1))
...
(define (roda-atras x)
  (cond
    ((and
      ...
      (not (member (xogo-estado x) (list "inicio" "cargando" "xogo->pantallas" "xogo->pantallas" "sel-pantallas-e" "edicion->atras" "edicion->nuevo")))
      )
     (cond
       ((or
